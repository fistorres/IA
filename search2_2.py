...
def graph_search(problem, frontier):
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            timespent = time()
            return [node,node.path(),visitados-1,len(explored),timespent - initial_time,node.path_cost]
        ...
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
                ...
                if problem.goal_test(child.state):
                    for i in child.path():
                        timespent = time()
                    return [child,child.path(),visitados,len(explored),timespent - initial_time,child.path_cost]
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            ...
            if child.state not in explored and child not in frontier:
                visitados += 1
                frontier.append(child)
                lenfront = max([lenfront,len(frontier)])
...
def depth_limited_search(problem, limit=50):
    ...
    def recursive_dls(node, problem, limit,visitados=0):
        if problem.goal_test(node.state):
            return node
        elif limit == 0:
            return 'cutoff'
            ...
            for child in node.expand(problem):
                # verifica se cada child é final ou se excede o limite
                visitados += 1
                result = recursive_dls(child, problem, limit - 1,visitados)
                if result == 'cutoff':
